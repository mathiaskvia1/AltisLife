...
if(isNil {GVAR_UINS "life_sql_id"}) then {
	...
	try {
		...
		if(!(EQUAL(_result,"[1]"))) then {throw "AtlanticServer: Error with Database Connection"};
	...
	EXTDB "9:LOCK";
	diag_log "AtlanticServer: Connected to Database";
} else {
	life_sql_id = GVAR_UINS "life_sql_id";
	CONSTVAR(life_sql_id);
	diag_log "AtlanticServer: Still Connected to Database";
};
...
[8,true,12] execFSM "\Atlantic_Server\FSM\timeModule.fsm";
...
[] call compile PreProcessFileLineNumbers "\Atlantic_Server\functions.sqf";
[] call compile PreProcessFileLineNumbers "\Atlantic_Server\eventhandlers.sqf";

/* Miscellaneous mission-required stuff */
[] spawn TON_fnc_cleanup;
life_wanted_list = [];
[] execFSM "\Atlantic_Server\FSM\cleanup.fsm";
